...
def _polyline(geometry_segments: list) -> list[tuple[float, float, float]]:
    """Flatten connected segments into ``(lat, lng, cumulative_m)`` points.

    Repeated coordinates are retained as route vertices but contribute no
    distance.  A segment whose first point is not the previous segment's last
    point is not joined: inventing a straight connector would put train
    positions off the supplied route.
    """
    ...
    for segment in geometry_segments:
        ...
        if not segment_points:
            continue
        if points and _haversine_m(points[-1][0], points[-1][1], segment_points[0][0], segment_points[0][1]) > 1.0:
            continue
    ...
    for lat, lng in points[1:]:
        distance = _haversine_m(line[-1][0], line[-1][1], lat, lng)
        if distance == 0.0:
            continue
        line.append((lat, lng, line[-1][2] + distance))
    return line


def _on_route(line: list[tuple[float, float, float]], target_m: float, position: tuple[float, float]) -> bool:
    """Ensure an interpolated point remains on its canonical route segment."""
    if not line or not all(math.isfinite(value) for value in (*position, target_m)):
        return False
    for previous, current in zip(line, line[1:]):
        if previous[2] <= target_m <= current[2] and current[2] > previous[2]:
            segment_length = current[2] - previous[2]
            ratio = (target_m - previous[2]) / segment_length
            expected = (
                previous[0] + (current[0] - previous[0]) * ratio,
                previous[1] + (current[1] - previous[1]) * ratio,
            )
            return _haversine_m(position[0], position[1], expected[0], expected[1]) <= 1.0
    return target_m in (0.0, line[-1][2])
...
def mrt_positions(
    ...
    for index, departure in enumerate(sorted({str(dep).strip() for dep in departures})):
        ...
        if position is None or not _on_route(line, target_m, position):
            continue
        distance_m = float(target_m)
        route_distance_m = float(total_m)
        if not math.isfinite(distance_m) or not math.isfinite(route_distance_m):
            continue
        distance_m = min(max(distance_m, 0.0), total_m)
        route_distance_m = min(max(route_distance_m, 0.0), total_m)
        next_station = next(
        ...
        trains.append(
            {
                ...
                "progress_pct": round(distance_m / total_m * 100, 1),
