...
const pickerSource = readFileSync(new URL('./src/StopPickerPage.tsx', import.meta.url), 'utf8')
...
const requiredContracts = [
  ...
  { label: 'stop search endpoint', value: '/api/gtfs/stops/search', source: pickerSource },
  ...
  // Moovit-style additions (trip-planner-moovit): ETA, incidents, saved places/history
  // The merge simplified arrival/departure into a single departure field:
  ...
  { label: 'departure time query param', value: "params.set('time', departureTime)", source: plannerSource },
...
for (const source of [plannerSource, trackingSource, mapSource, storageSource, pickerSource]) {
